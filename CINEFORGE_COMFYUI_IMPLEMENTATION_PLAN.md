...
**Date:** 2026-07-15<br>
**Status:** Default-off local execution-control software implemented and mock-validated; hardware benchmark, recovery/OOM exercise, expanded graph admission, and human QA evidence remain required before production-readiness claims<br>
...
Public and autonomous generation remain out of scope and disabled: no public raw `/prompt` proxy, no internet-facing semantic generation endpoint, no job-initiated installs/downloads, and no automatic generation from Storyboard approval. Local operator-only FFmpeg, ComfyUI, and GPU-heavy actions may be considered only after explicit operator approval for the relevant mode/run, using the approval shape in `docs/LOCAL_OPERATOR_LIVE_BOUNDARY.md`, and after the applicable gates are satisfied: safe managed paths, admitted graph/model/profile pins, local-only storage, default-off operator gates, exclusive GPU lease for Comfy/GPU work, queue-state checks, provenance/audit capture, recovery/stop rules, and human QA where required. This checkpoint does not approve or perform any live FFmpeg, ComfyUI, GPU, render, or benchmark action, and it does not claim runtime readiness.

Current read-only local MVP surfaces now exist: `GET /local-runtime/local-mvp-readiness`, fail-closed `GET /local-runtime/public-readiness`, `GET /local-runtime/checkpoint-watchdog`, read-only M6/M7 readiness rollups, offline semantic request manifests, explicit Storyboard-to-offline-semantic handoffs, and Runtime page panels for checkpoint watchdog, local/public readiness, plus stored operator review packets. These surfaces are inert and evidence-only: they do not create live jobs, approve runs, submit ComfyUI work, execute FFmpeg/ffprobe, acquire a GPU lease, render media, benchmark, or mark any profile/preset public-ready. `GET /local-operator/approval-templates` exposes exact approval-shape examples and non-approval phrases; `GET /local-operator/runbooks` exposes read-only M4/M5 prerequisites, stop rules, and evidence expectations; `POST /local-operator/packets` creates pending review metadata only and always records no approval or execution. `scripts/validate_safe_local_boundary.py`, `scripts/run_offline_safe_validation.py`, and the GitHub Actions offline-safe validation workflow now make the static no-live-boundary checks reproducible. The checkpoint watchdog is deployed in the runner, CI, API, Runtime UI, and docs: after every checkpoint it prints/serves the restart directive and no-live invariants, includes the rule that watchdog/status answers are not stopping points, supports optional JSON output plus clean tracked-tree/source-scoped untracked-file enforcement, and is guarded by docs/route/static tests plus raw-prompt/workflow-fragment static checks. M4 live hardware probing/benchmarking and M5 live FFmpeg/ffprobe/assembly remain blocked until explicit operator approval for the exact next run; no live execution has been performed by these readiness checkpoints. Public release hardening and actual expanded preset enablement remain deferred. The latest curated offline-safe runner reports static boundary validation passed, `308 passed`, frontend lint/build passed, `git diff --check` passed, and the checkpoint watchdog banner printed; no live runtime/media action is included in that suite.

### Default-off execution-control checkpoint (2026-07-21)

The local software execution path is now implemented without enabling it by default. Passive and manifest-only contracts remain under `/local-*`; actions that can contact or mutate a runtime are separated under `/operator-runtime/*`, `/operator-generation/*`, and `/operator-post-production/*`. All operator, queue-worker, auto-start, and FFmpeg gates default to false.

Implemented software controls now include:

- pinned external ComfyUI process ownership with passive status, explicit probe/start/restart/stop, bounded readiness checks, ownership records, and refusal to stop an unowned process;
- durable semantic request compilation and queue creation with immutable job lineage;
- one backend-controlled worker path with mandatory live `/object_info` validation, exclusive GPU lease, controlled prompt submission, WebSocket progress, history/output collection, managed-path checks, SHA-256 provenance, and terminal state persistence;
- bounded transient retry and stale reserved/active job recovery without mutating prior job history;
- cancellation cleanup through the runtime client;
- stale-output rejection using the job submission timestamp;
- gated FFmpeg/ffprobe execution for persisted allowlisted plans/recipes, including argv and input-hash revalidation, no-overwrite behavior, pre-execution job persistence, final probe validation, and output SHA-256 evidence;
- frontend acknowledgement, requested-by audit fields, honest passive loading, and explicit operator buttons for the above controls;
- deterministic fake-client/process tests that exercise the full worker lifecycle, retry behavior, runtime default-off/ownership behavior, and FFmpeg success/default-off/hash-change behavior without contacting ComfyUI, GPU hardware, or FFmpeg.

This checkpoint does **not** enable public/autonomous generation, does not admit every image/video/control archetype, and does not prove the RTX 5090 Laptop benchmark ladder or human visual/audio quality. No image or video generation was run while implementing or validating this checkpoint. I2V/control/lipdub asset staging remains unavailable until those archetypes have admitted bindings and required live evidence. M4 hardware evidence and M6 human QA therefore remain genuine activation blockers, not software status claims.
...
- Controlled history/WebSocket/output collection and the end-to-end worker are implemented behind independent default-off gates; no production hardware evidence has been recorded.
...
- M5 has deterministic planning, allowlisted recipe builders, a gated executor, job/probe/hash persistence, and mocked execution coverage, but no approved live FFmpeg/ffprobe execution evidence.
