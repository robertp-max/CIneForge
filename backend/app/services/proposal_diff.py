...
def _identity_key(item: Any, index: int) -> str:
    if isinstance(item, dict):
        # Existing proposal entities carry both a temporary client_id and the
        # persisted existing_id.  Prefer the persisted identity so they align
        # with canonical snapshots (which use id) instead of appearing as a
        # misleading remove+add pair.
        for key in ("existing_id", "id", "client_id"):
        ...
        if "story" in base_snapshot or "chapters" in base_snapshot:
            base_story = {
                **(base_snapshot.get("story") or {}),
                ...
                "voices": (
                    base_snapshot.get("voice_profiles")
                    if "voice_profiles" in base_snapshot
                    else base_snapshot.get("voices")
                )
                or [],
