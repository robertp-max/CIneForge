...
2. Curated backend tests for local catalogs, local jobs, offline generation manifests, readiness reports, operator packets/runbooks/templates, post-production manifests, production gates, CF-VID-01 workflow-template contracts, and workflow admission/registry logic.
3. Frontend lint and build.
4. `git diff --check`.

Current checkpoint result: `167 passed`, frontend lint/build passed, static boundary validation passed, endpoint-matrix path/method sync passed, offline docs boundary checks passed, and the checkpoint watchdog banner printed.
