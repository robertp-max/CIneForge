...
def parse_implementation_result(envelope: object, expected_session_id: str) -> ImplementationResult:
    ...
    if not isinstance(envelope["text"], str):
        raise ValueError("Grok envelope text must be a JSON string")
    try:
        payload = json.loads(envelope["text"])
    except json.JSONDecodeError as error:
        raise ValueError("Grok envelope text is not valid JSON") from error
    ...
    if status not in {"completed", "blocked", "failed"} or not isinstance(summary, str):
    ...
    for raw in raw_patches:
        ...
        if action not in {"create", "replace"} or not isinstance(content, str) or "\0" in content:
    ...
    return ImplementationResult(
        ...
        findings=_string_list(payload["findings"], "findings", 256),
        tests=_string_list(payload["tests"], "tests", 64),
        blockers=_string_list(payload["blockers"], "blockers", 64),
    )
